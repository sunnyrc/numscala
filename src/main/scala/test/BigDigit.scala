...
object BigDigit {
  ...
  def load(): (DenseVector[Int], DenseMatrix[Double]) = {
    val f1 = Source.fromInputStream(gis(getClass.getResourceAsStream("/data/digits_train.csv.gz")))
    val train = f1.getLines().toList.map(_.split(",").map(_.toDouble))

    val f2 = Source.fromInputStream(gis(getClass.getResourceAsStream("/data/digits_labels.csv.gz")))
